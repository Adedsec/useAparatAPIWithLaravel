...
class AparatHandler
{
    ...
    public function upload(string $filename, string $title, int $category)
    {
        ...
        $uploadResponse = $this->http::attach('video', file_get_contents(Storage::disk('public')->path($filename)), $filename)
            ->post($formActionUrl, [
                [
                    'name' => 'frm-id',
                    'contents' => $formId
    ...
    public function show(string $uid)
    {
        ...
        try {
            ...
            $response = $this->http::get($url);
            if (!array_key_exists('id', $response->json()['video'])) {
    ...
    private function replaceParams(string $url, $options = [])
    {
